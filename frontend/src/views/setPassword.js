...
export default function SetPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        Notification('success', 'Şifreniz başarıyla oluşturuldu!', 'Kullanıcı bilgileriniz ile giriş yapabilirsiniz')
    };
    return (
        <div className='flex '>
            ...
            <div className='flex flex-col justify-center items-center h-screen bg-white w-3/5'>
                <Form onSubmit={handleSubmit} className='max-w-[458px] w-full mx-auto '>
                    ...
                    <p className='text-sm text-left my-4  font-normal'>Lütfen şifre kurallarına uygun olarak şifrenizi belirleyiniz.</p>
                            ...
                            <Alert
                                ...
                                description={
                                    (password.length >= 8 && /[a-zA-Z]/.test(password) && /\d/.test(password)) ?
                                        ...
                                        <>
                                            {password.length < 8 && <p>En az 8 karakter olmalıdır.</p>}
                                            {!/[a-zA-Z]/.test(password) && <p>En az bir harf içermelidir.</p>}
                                            {!/\d/.test(password) && <p>En az bir rakam içermelidir.</p>}
                                        </>
                                }
                                type={
                                    (password.length >= 8 && /[a-zA-Z]/.test(password) && /\d/.test(password)) ? "success" : "warning"
                                }
                                showIcon
                            />
                        </div>
                        <div className='flex row justify-between mt-2 mb-2'>
                            {password === confirm && password.length >= 8 && /[a-zA-Z]/.test(password) && /\d/.test(password) ? (
                                <button
                                    type='submit'
                                ...
                                <Button disabled={true} className='h-9 w-full mt-5'>Şifre Belirle</Button>
                            )}
                        </div>
